...
distancePose DistanceReset::calculateGlobalPosition(
  ...
  for (const auto &sensor : active_sensors) {
    ...
    double global_angle_deg =
        std::fmod(heading_deg + sensor.mounting_angle, 360.0);
    if (global_angle_deg < 0)
      global_angle_deg += 360.0;

    double global_offset_x =
        (sensor.forward_offset * sin_h) + (sensor.strafe_offset * cos_h);
    double global_offset_y =
        (sensor.forward_offset * cos_h) - (sensor.strafe_offset * sin_h);

    double diff_0 = std::min(std::fabs(global_angle_deg - 0.0),
                             std::fabs(global_angle_deg - 360.0));
    ...
    if (diff_0 <= default_heading_tolerance && use_pos_y_wall) {
      double angle_err_rad = (global_angle_deg > 180 ? global_angle_deg - 360.0
                                                     : global_angle_deg) *
                             (M_PI / 180.0);
      y_cands.push_back(HALF_HEIGHT - (dist_in * std::cos(angle_err_rad)) -
                        global_offset_y);
    } else if (diff_90 <= default_heading_tolerance && use_pos_x_wall) {
      double angle_err_rad = (global_angle_deg - 90.0) * (M_PI / 180.0);
      x_cands.push_back(HALF_WIDTH - (dist_in * std::cos(angle_err_rad)) -
                        global_offset_x);
  ...
  if (!x_cands.empty()) {
    est_x =
        std::accumulate(x_cands.begin(), x_cands.end(), 0.0) / x_cands.size();
    using_odom_x = false;
  }
  if (!y_cands.empty()) {
    est_y =
        std::accumulate(y_cands.begin(), y_cands.end(), 0.0) / y_cands.size();
    using_odom_y = false;
  }
...
DistanceReset::DistanceReset(Chassis *robot_chassis,
    ...
    : chassis(robot_chassis), sensors(robot_sensors),
...
DistanceReset::updateSpecific(const std::vector<DistanceSensor> &active_sensors,
  ...
  distancePose pose = calculateGlobalPosition(active_sensors, current.theta,
                                              current.x, current.y);
